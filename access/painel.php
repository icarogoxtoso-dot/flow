...
$schemaFile = __DIR__ . '/servicos_db';
...
try {
    ...
    error_log('DB error em painel.php: ' . $e->getMessage());
...
<body class="min-h-screen text-slate-900 bg-slate-50 selection:bg-blue-200 selection:text-blue-900">
    ...
    <script>
        ...
        const professionals = <?php echo json_encode($profissionais_db, JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES | JSON_HEX_TAG | JSON_HEX_AMP | JSON_HEX_APOS | JSON_HEX_QUOT); ?>;
        ...
        function renderCards(data) {
            ...
            data.forEach((p, index) => {
                ...
                const photoUrl = sanitizeImageUrl(p.foto_perfil);
                const location = escapeHtml([p.bairro, p.cidade].filter(Boolean).join(' - ') || 'Local não informado');
                const startYearRaw = String(p.desde ?? '').trim();
                const startYear = /^[0-9]{4}$/.test(startYearRaw) ? startYearRaw : '-';
                ...
                const safePublicId = /^[a-f0-9]{32}$/.test(String(p.public_id || '')) ? String(p.public_id) : '';
                ...
                card.innerHTML = `
                    <div class="grid grid-cols-1 xl:grid-cols-[1fr_auto] gap-4 items-end">
                        ...
                        <div class="w-20 h-20 rounded-full border border-gray-200 bg-gray-100 overflow-hidden flex items-center justify-center text-gray-400 font-bold text-lg shrink-0">
                            ${photoUrl ? `<img src="${escapeHtml(photoUrl)}" alt="Foto de ${displayName}" class="w-full h-full object-cover">` : initials}
                        </div>
